...
class GNN(nn.Module):
    ...
    def compute_loss(self, pos_list, extra_output, batch, args):    # 计算损失
        loss_dict = {}
        loss = 0

        pos = batch.pos

        # new_idx = GNN.update_iso(pos, pos_list[-1], batch)  # todo：S2的解，置换。测试是否和论文中一样
    ...
    @staticmethod
    def alignment_loss(pos_y, pos_x, batch, clamp=None):    # 对齐的loss,s1
        with torch.no_grad():
    ...
    @staticmethod
    def update_iso(pos_y, pos_x, batch):    # todo：s2的解
        with torch.no_grad():
            pre_nodes = 0
            num_nodes = batch.n_nodes
